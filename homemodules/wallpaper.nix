...
in
{
  ...
  options.wallpaper = {
    enable = lib.mkEnableOption "enables the wallpaper module";
    ...
    backgroundImageOpacity = lib.mkOption {
      ...
      apply =
        ...
        lib.throwIf (
          value < 0 || value > 1
        ) "wallpaper.backgroundImageOpacity value should be between 0 and 1" value;
    };
    ...
    iconScale = lib.mkOption {
      ...
      apply =
        value: lib.throwIf (value < 0) "wallpaper.iconScale value should be a positive float." value;
    };
  };
  ...
  config = lib.mkIf config.wallpaper.enable {
    ...
    home.file."${wallpaperPath}/nixos_logo.svg" = lib.mkIf (!config.wallpaper.ultrawide) {
      text =
        let
          bgImage =
            if (config.wallpaper.backgroundImage != null) then config.wallpaper.backgroundImage else "";
          primaryColor = "#8255c2";
    ...
    programs =
      if hasPlasmaManager then
        {
          plasma.enable = true;
          plasma.workspace.wallpaper =
            if config.wallpaper.ultrawide then
              "${wallpaperPath}/nixos_logo_ultrawide.svg"
            else
              "${wallpaperPath}/nixos_logo.svg";
